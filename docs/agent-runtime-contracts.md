...
- Runtime exposes the live AgentService to its authenticated desktop host at `POST /v1/agent/runs`,
  ...
  identity rather than accepting it from task text. IM is not advertised until its trusted adapter
  exists. Host-event transport remains TASK-015. Every submission carries the contract version and preserves the
