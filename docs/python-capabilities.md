...
TASK-007 已在现有技能页和对话页实现该流程：安装前展示来源、发布者、权限及进程边界；更新失败保留并显示活动版本，支持重试和历史版本回滚；敏感调用以内联卡片提供“允许一次/拒绝”，刷新后由 Runtime 的待审批记录恢复。旧 `pi-mcp-adapter` 与 Yuanpu 进程外能力发生托管冲突时必须由用户明确选择，停用旧适配器不会删除其独立配置。交互方案和浏览器证据保存在 `.tasks/ui/task-007-skill-ui/`。
