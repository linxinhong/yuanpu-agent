...
| V19-06 企业微信真实收发 | 授权测试成员通过首发智能机器人完成双会话、重复消息、回复和故障恢复。 | **unverified（真实收发）**。用户核对并更新 `.env` 后，SDK 1.0.7 单次鉴权探针返回 `authenticated=true`；此前的 `853000` 是旧配置的历史失败，已不再是当前鉴权阻断。未读取或记录变量值、原始错误、消息和真实标识，也未发送消息。本机配置仍为 1 条连接、0 启用、0 配对、0 Keychain 引用；目前只有一个获授权私聊会话，且 `WECOM_TEST_USER_ID` 尚未配置，故单会话收发尚未开始，双会话仍缺第二成员。 |
...
- 用户随后核对并更新同一 `.env`；2026-09-23 复跑同一命令得到 `{"authenticated":true}`，退出码 0。`WECOM_TEST_USER_ID` 的布尔存在检查返回 false，因此没有启动真实收发或向任何会话试发。已准备 `packages/yuanpu-runtime/test/task-019-wecom-live-probe.mjs`：只接受该变量指定的单人私聊且正文等于随机 challenge，执行固定回复，记录脱敏状态与临时 SQLite 事实；尚未运行，不计入 pass 证据。
...
D19-01 是 TASK-019 的阻断缺陷：需要实现 Runtime 对已绑定 IM route 的计划投递授权与渠道 delivery adapter，明确未知发送不自动重发且失败不重跑 Agent。该实现属于开发修复，不在本验收卡内。修复后先重跑 API 探针并补充一个真实 SQLite + 渠道投递失败/未知的组合场景，再重跑 `pnpm check`。新 Bot 已通过鉴权；待用户在原 `.env` 配置唯一获授权测试成员的 `WECOM_TEST_USER_ID` 后，先在临时隔离连接中执行单会话收发，不触动现有本机连接。双会话需要第二名获授权测试成员。最后用真实 Electron 完成 V19-07；通过前不得标记 TASK-019 完成或解锁管理界面卡。
