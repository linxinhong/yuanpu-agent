...
下列命令在 macOS arm64、Node 24.15.0、pnpm 11.22.0 的候选树运行。`coding-owner` runner 记录命令、退出码和候选树指纹；提交 `da8e488` 与受测脏工作树的文件内容相同，提交前 `git diff --cached --check` 通过。所有测试数据在临时目录中，未使用真实机器人、模型密钥或个人 App 数据。
