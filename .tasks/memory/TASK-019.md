...
- 仅有一个获授权私聊测试会话，`WECOM_TEST_USER_ID` 尚未配置；`packages/yuanpu-runtime/test/task-019-wecom-live-probe.mjs` 已准备为临时隔离、仅匹配指定成员与随机 challenge 的真实收发探针，尚未运行。双会话仍缺第二成员。鉴权脚本提交 `6ce1d60` 的 `pnpm check` 已通过（runner `1790128038312650000.json`）；当前阻断项是 D19-01 以及真实 IM/Electron 业务证据。
...
- 下一步：开发修复 D19-01 并在集成 revision 重跑 API 探针、渠道失败/未知组合与 `pnpm check`；用户设置唯一获授权成员的 `WECOM_TEST_USER_ID` 后运行临时单会话探针，第二成员到位后补齐双会话与真实 Electron 验收。不要把 fixture 通知 `submitted` 写成用户已看到。
