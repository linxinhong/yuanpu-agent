...
升级失败保持活动版本并显示重试；历史版本仅显式回滚。Runtime 先验签 manifest，再把其权限、配置 schema 和连接名用于 UI/冲突判断；只有连接名与旧 `pi-mcp-adapter` 配置重合才要求用户选择，停用不会删除配置。能力配置存放于版本目录之外，Python MCP 每次调用读取，因此更新/回滚均保留且无需重启。PyInstaller 冻结入口必须在导入 MCP 依赖前处理 `--version`；macOS 冷启动仍可能接近 10 秒，安装健康检查使用 20 秒上限。
